...
_custom-bash-comp-list-enabled()
{
	subdirectory="$1"
...
_custom-bash-comp-list-available()
{
	subdirectory="$1"
    ...
    update | search)
      ...
      ;;
		enable | disable)
			...
			case "${file_type}" in
				alias)
						_custom-bash-comp-list-${suffix} aliases
						return 0
						;;
				plugin)
						_custom-bash-comp-list-${suffix} plugins
						return 0
						;;
				completion)
						_custom-bash-comp-list-${suffix} completion
						return 0
						;;
				*)
						_custom-bash-comp-enable-disable
		...
		aliases)
			...
			case "${prevprev}" in
				help)
					_custom-bash-comp-list-available aliases
					return 0
					;;
			esac
			;;
...
complete -F _custom-bash-comp custom-bash
